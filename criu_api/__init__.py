...
def set_dumps_dir(dumps_dir: str | Path = 'criu_dumps/'):
    ...
    if isinstance(dumps_dir, str):
        ...
        raise TypeError(f"dumps_dir must be str or Path, not {type(dumps_dir).__name__} ({dumps_dir})!")
...
def dump(dump_number: int = None, leave_running = True, allow_overwrite = False, additional_args: list[str] = None):
    ...
    if dump_number:
        if not isinstance(dump_number, int) or dump_number < 0:
            raise ValueError(f'dump_number ({repr(dump_number)}) should be int >= 0.')
        if dump_number > _last_dump_number + 1:
            raise ValueError(
                f'dump_number ({dump_number}) must be <= {_last_dump_number + 1}.'
            )
        if dump_number <= _last_dump_number:
            if allow_overwrite:
                ...
                raise FileExistsError(f'Dump {dump_number} already exists! Consider using allow_overwrite=True.')
    else:
